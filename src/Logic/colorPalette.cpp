...
colorPalette::colorPalette(string name, double left, double right) : paletteName{std::move(name)},
...
rgb colorPalette::getColor(double x) {
    ...
    if (left >= 1 && right < palette.size() - 1) {
        return rgb{
                (palette[left][0] * ls + palette[right][0] * rs) * 2 / 3 +
                (palette[left - 1][0] * ls + palette[right + 1][0] * rs) * 1 / 3,
                (palette[left][1] * ls + palette[right][1] * rs) * 2 / 3 +
                (palette[left - 1][1] * ls + palette[right + 1][1] * rs) * 1 / 3,
                (palette[left][2] * ls + palette[right][2] * rs) * 2 / 3 +
                (palette[left - 1][2] * ls + palette[right + 1][2] * rs) * 1 / 3,
        };
    }
    return rgb{
